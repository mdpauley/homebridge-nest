...
NestThermostatAccessory.prototype.setCoolingThresholdTemperatureDebounced = debounce(function (targetTemperature, callback) {
  const setting = this.device.hvac_mode === 'heat-cool' ? 'target_temperature_high_' : 'target_temperature_';
  return this.updateTargetWithCorrectUnitsAsync(setting, targetTemperature, "cooling threshold temperature").asCallback(callback);
}, 5000);

NestThermostatAccessory.prototype.setHeatingThresholdTemperatureDebounced = debounce(function (targetTemperature, callback) {
  const setting = this.device.hvac_mode === 'heat-cool' ? 'target_temperature_low_' : 'target_temperature_';
  this.updateTargetWithCorrectUnitsAsync(setting, targetTemperature, "heating threshold temperature").asCallback(callback);
}, 5000);
